...
-- Create a profiles table to extend user information (since we can't modify auth.users directly)
...
CREATE POLICY "Line managers can view assigned requests" ON requests
    FOR SELECT USING (
        ...
        auth.uid() IN (
            ...
            WHERE p.line_manager_id = auth.uid() 
            AND p.id = requests.user_id
        )
    );
...
SELECT 
    ...
    lmp.full_name as line_manager_name,
...
LEFT JOIN profiles lmp ON r.line_manager_id = lmp.id
LEFT JOIN auth.users approver ON r.approved_by = approver.id
LEFT JOIN profiles ap ON r.approved_by = ap.id;
